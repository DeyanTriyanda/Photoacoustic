...
std::string win_error_message() {
    const DWORD err = GetLastError();
    char* buf = nullptr;
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                       FORMAT_MESSAGE_IGNORE_INSERTS,
                   nullptr, err, 0, reinterpret_cast<LPSTR>(&buf), 0, nullptr);
    std::string msg = buf ? buf : ("Win32 error " + std::to_string(err));
    if (buf) LocalFree(buf);
    while (!msg.empty() && (msg.back() == '\n' || msg.back() == '\r')) msg.pop_back();
    return msg;
}

DWORD baud_to_win(int baudrate) {
    switch (baudrate) {
...
std::vector<PortInfo> SerialController::list_ports() {
    ...
    for (int i = 1; i <= 256; ++i) {
        const std::string device = "COM" + std::to_string(i);
...
std::pair<bool, std::string> SerialController::connect(const std::string& port,
    ...
    HANDLE h = CreateFileA(path.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
                           OPEN_EXISTING, 0, nullptr);
    if (h == INVALID_HANDLE_VALUE) {
        return {false, "Gagal membuka port: " + win_error_message()};
    }
    ...
    if (!GetCommState(h, &dcb)) {
        CloseHandle(h);
        return {false, "GetCommState gagal: " + win_error_message()};
    }
    dcb.BaudRate = baud_to_win(baudrate);
    dcb.ByteSize = 8;
    dcb.Parity = NOPARITY;
    ...
    if (!SetCommState(h, &dcb)) {
        CloseHandle(h);
        return {false, "SetCommState gagal: " + win_error_message()};
    }
...
std::pair<bool, std::string> SerialController::send(const std::string& command) {
    std::lock_guard<std::mutex> lock(io_mutex_);
    if (handle_ == INVALID_HANDLE_VALUE) return {false, "Belum terhubung ke Arduino"};
    ...
    payload.push_back('\n');
    DWORD written = 0;
    if (!WriteFile(handle_, payload.data(), static_cast<DWORD>(payload.size()), &written,
                   nullptr)) {
        return {false, "Gagal kirim: " + win_error_message()};
    }
...
void SerialController::read_loop() {
    ...
    while (running_.load()) {
        ...
        if (!ReadFile(h, buf, sizeof(buf), &n, nullptr)) {
            const DWORD err = GetLastError();
            if (err == ERROR_IO_PENDING || err == ERROR_TIMEOUT) {
            ...
            if (msg_cb) msg_cb("[ERROR BACA SERIAL] " + win_error_message());
            break;
        }
        if (n == 0) {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
