...
Dua jalur di atas stream yang sama:
  1. Rolling buffer GUI: get_waveform / get_fft / get_peak
  2. Blok baru on-demand: capture_samples / compute_fft / capture_and_measure

list_input_devices(force_rescan=True) melakukan terminate+initialize PortAudio
supaya device yang dicolok setelah app dibuka ikut terdeteksi (hanya aman
saat tidak ada stream aktif).
"""
...
def _sd():
    """Lazy-import sounddevice (butuh PortAudio di sistem)."""
    try:
...
class AudioCapture:
    ...
    def list_input_devices(force_rescan=True):
        """List (index, label) device INPUT. force_rescan reinit PortAudio."""
        try:
            sd = _sd()
        except RuntimeError:
            return []
    ...
    def start(self, device_index, samplerate=96000, channels=1,
        ...
        """Mulai streaming. buffer_seconds = jendela rolling GUI saja."""
        if self.running:
            self.stop()
        ...
        def _callback(indata, frames, time_info, status):
            ...
            # Baca flag di dalam lock supaya konsisten dengan capture_samples.
            with self._capture_lock:
    ...
    def compute_fft(self, data, window="hann", min_freq=0.0, max_freq=None):
        """FFT dari array yang diberikan (hasil capture_samples)."""
        return self._fft_core(
    ...
    def get_peak(self, min_freq=20.0, max_freq=None):
        """(freq_puncak_hz, amplitudo) dari rolling buffer."""
    ...
    def capture_samples(self, n, timeout=2.0):
        """
        Blocking: kumpulkan N sampel BARU dari callback (bukan rolling buffer).
        Panggil dari worker thread; satu capture aktif dalam satu waktu.
        """
    ...
    def capture_and_measure(self, n, target_freq_hz, freq_tolerance_hz=50.0,
        ...
        """capture_samples + compute_fft + ekstraksi amplitudo di target."""
